...
function getProductDetails($pdo, $product_id) {
    try {
        $stmt = $pdo->prepare("SELECT p.product_id, p.name, p.price, p.image_url, p.allergens, COALESCE(i.stock_level, 0) as stock_level 
                               FROM product p 
...
function validatePromoCode($pdo, $code) {
    $today = date('Y-m-d');
    try {
        $stmt = $pdo->prepare("SELECT * FROM promotion WHERE code = ? AND start <= ? AND end >= ? AND type = 'percentage'");
...
try {
    if ($action === 'get_cart_html') {
        ...
        <div class="cart-items-modal">
            ...
            <?php else: ?>
                <?php foreach ($_SESSION['cart'] as $index => $item):
                    $product = getProductDetails($pdo, $item['item_id'] ?? $item['id'] ?? '');
                    if (!$product) continue;
        ...
        if ($action === 'update') {
            ...
            $product = getProductDetails($pdo, $_SESSION['cart'][$index]['item_id']);
            if ($quantity > $product['stock_level']) {
        ...
        if ($action === 'remove') {
            ...
            $product_id = $_SESSION['cart'][$index]['item_id'];
        ...
        if ($action === 'apply_promo') {
            ...
            $discount = validatePromoCode($pdo, $code);
            if ($discount > 0) {
        ...
        if ($action === 'add_wish') {
            $product_id = $_POST['item_id'] ?? $data['item_id'] ?? '';
